...
def strain_selection(distance_data, seq_data):
    ...
    for i in range(0, distance_data.shape[0]):
        ...
        for j in range(0, seq_data.shape[0]):
            if str(seq_data['description'].iloc[j]) == str(distance_data['Strain1'].iloc[i]):
                seq1 = str(seq_data['seq'].iloc[j]).upper()
                flag1 = 1
            if str(seq_data['description'].iloc[j]) == str(distance_data['Strain2'].iloc[i]):
...
def bayes_cross_validation(train_x, train_y, valid_x, valid_y):
    np.random.seed(100)
    clf = GaussianNB()
    clf.fit(train_x, train_y)

    train_acc = accuracy_score(train_y, clf.predict(train_x))
    train_pre = precision_score(train_y, clf.predict(train_x))
    train_rec = recall_score(train_y, clf.predict(train_x))
    train_fscore = f1_score(train_y, clf.predict(train_x))
    train_mcc = matthews_corrcoef(train_y, clf.predict(train_x))
    
    Y_pred = clf.predict(valid_x)
    precision, recall, fscore, mcc, val_acc = evaluate(valid_y, Y_pred)
    
    t_ans = '%.3f,%.3f,%.3f,%.3f,%.3f\n'% (train_acc, train_pre, train_rec, train_fscore, train_mcc)
    v_ans = '%.3f,%.3f,%.3f,%.3f,%.3f\n'% (val_acc, precision, recall, fscore, mcc)
    return t_ans,v_ans

def bayes_cross_validation_test(train_x, train_y, valid_x, valid_y):
    np.random.seed(100)
    clf = GaussianNB()
    clf.fit(train_x, train_y)
    Y_pred = clf.predict(valid_x)
    return Y_pred,valid_y

def bayes_cross_validation_test_ROC(train_x, train_y, valid_x, valid_y):
    np.random.seed(100)
    clf = GaussianNB()
    clf.fit(train_x, train_y)
    Y_pred = clf.predict(valid_x)
    prob_y = clf.predict_proba(valid_x)
    temp = tf.keras.utils.to_categorical(valid_y)
    print(temp)
    return prob_y,temp 
...
def svm_baseline_test(X, Y, X_test, Y_test, method=None):
    setup_seed(20)
    clf = SVC(gamma='auto', class_weight='balanced').fit(X, Y)    
    Y_pred = clf.predict(X_test)
    temp = []
...
def svm_baseline_test_ROC(X, Y, X_test, Y_test, method=None):
    setup_seed(20)
    Y_test = tf.keras.utils.to_categorical(Y_test.values)
...
def rf_baseline_test(X, Y, X_test, Y_test):
    setup_seed(20)
    clf = ensemble.RandomForestClassifier().fit(X, Y) 
    Y_pred = clf.predict(X_test)
    temp = []
