...
def Du_train_5folds():
    ...
    save = './training_data_5folds/Du_Xiangjun/'
    for years in range(0,5):
        f1 = open(save+"H3N2_seq_{}_{}".format("train",years),"w")
        f2 = open(save+"seqCompare_H3N2_{}_{}".format("train",years),"w")
        ll = []
        for jj in range(5):
            if(jj!=years):
                ll.append(jj)
        for year in ll:
            try:
                Relation = "./CNN_nodup_years/5folds/{}.csv".format(year)
                H3N2_Relation = pd.read_csv(Relation)
                seq = './sequence_5folds/data/{}.csv'.format(year)
                H3N2_seq = pd.read_csv(seq,names=['seq', 'description'])
...
def main():
    for i in range(5):
        if(True):
            print("Year: {} ".format(i))
            Antigenic_dist = './antigen_5folds/data/{}.csv'.format(i)
            seq = './sequence_5folds/data/{}.csv'.format(i)
            #William(Antigenic_dist,seq,'./training_data_5folds/William Lees/{}.csv'.format(str(i)))
            #Peng(Antigenic_dist,seq,'./training_data_5folds/Peng Yousong/{}.csv'.format(str(i)))
            #Liao(Antigenic_dist,seq,'./training_data_5folds/Yu-Chieh Liao/{}.csv'.format(str(i)))
            #Yao(Antigenic_dist,seq,'./training_data_5folds/Yuhua Yao/{}.csv'.format(str(i)))
            #IAV(Antigenic_dist,seq,'./training_data_5folds/IVA-CNN/{}_x.npy'.format(str(i)),'./training_data_5folds/IVA-CNN/{}_y.npy'.format(str(i)))
            Relation = "./CNN_nodup_years/5folds/{}.csv".format(i)
            Du(Relation,seq,'./training_data_5folds/Du_Xiangjun/',i)
            continue
Du_train_5folds()
