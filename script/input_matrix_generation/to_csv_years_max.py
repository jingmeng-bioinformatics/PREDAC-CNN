import pandas as pd
filedata = pd.read_csv("./All_data_add_smith_no_conflict_double.csv")
f = open("./junk.txt","w")
count = 0
...
for i in range(ll):  #   if i == ll-1 , data = all , we dont need it  ； data is i ,but name is i+1
    try:
        years_data[list_years[i]].to_csv("../csv/add_smith_no_conflit_double/{}.csv".format(list_years[i]),index=False)
    except:
        print("error")
